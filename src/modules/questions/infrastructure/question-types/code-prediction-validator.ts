...
export class CodePredictionValidator implements QuestionValidator {
  validate(question: Question): ValidationResult {
    const errors: string[] = [];
    if (!question.options || question.options.length < 2) {
      errors.push("Code-prediction questions require at least 2 output options.");
    }
  ...
  validateSeed(seed: QuestionSeed): ValidationResult {
    ...
    if (!seed.options || seed.options.length < 2) {
      errors.push("Code-prediction seeds require at least 2 output options.");
    }
    return { isValid: errors.length === 0, errors };
  }
}
